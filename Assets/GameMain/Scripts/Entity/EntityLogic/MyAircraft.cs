...
namespace StarForce
{
    public class MyAircraft : Aircraft
    {
        ...
        private Vector3 m_TransDirection = Vector3.zero;
...
#endif
        {
            ...
            Vector3 my_dir = Camera.main.ScreenToWorldPoint(Input.mousePosition) - transform.position;
            m_TransDirection.y = 180 * Mathf.Atan2(my_dir.x, my_dir.z)/Mathf.PI; 

            transform.rotation = Quaternion.Euler(m_TransDirection);
            m_Thruster.transform.rotation = transform.rotation;
            ...
            if (Input.GetMouseButton(0))
            {
                ...
                {
                    m_Weapons[i].TryAttack( new Vector3(my_dir.x, 0 , my_dir.z).normalized);
                }
            }
